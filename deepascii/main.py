...
def main(config):
    ...
    charset_chars = string.ascii_letters + string.digits + """\/|-=+@#%*~`. """
    charset_imgs = generate_charset(charset_chars, config.font, config.font_size)
    print('Extracting features from charset...')
    features = extract_charset_features(charset_imgs)
    print('Creating network...')
    model = make_model(source_arr.shape, features)
    model.compile(loss='categorical_crossentropy', optimizer='adam')
    print('Predicting...')
    z = model.predict(source_arr[None, ...])
